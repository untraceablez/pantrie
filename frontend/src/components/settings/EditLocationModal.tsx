...
import { updateLocation, type Location } from '@/services/location'
...
export default function EditLocationModal({ location, onClose, onSuccess }: EditLocationModalProps) {
  ...
  useEffect(() => {
    if (location.icon && !commonIcons.includes(location.icon)) {
      setCustomIcon(location.icon)
      setIcon('')
    } else {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 dark:bg-opacity-70 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg max-w-md w-full">
        <div className="border-b border-gray-200 dark:border-gray-700 px-6 py-4 flex justify-between items-center">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">Edit Location</h2>
          <button
            onClick={onClose}
